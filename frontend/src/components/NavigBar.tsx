...
function NavigBar() {
  ...
  const linkStyle = (path: string) => {
    ...
    return `flex flex-col md:flex-row items-center justify-center md:justify-start gap-1 md:gap-4 group no-underline transition-all w-full md:w-full rounded-lg
    px-1 py-1.5 md:px-3 md:py-3
    ${active
      ? 'bg-slate-800 text-blue-400 md:text-white md:shadow-lg scale-105 md:scale-100 md:origin-left'
      : 'text-slate-400 hover:bg-slate-800 hover:text-white'}
    `;
  };

  const textStyle = `hidden md:block text-[10px] md:text-xs lg:text-[0.9rem] uppercase font-bold tracking-wider transition-all overflow-hidden whitespace-nowrap ${isBurgerOpen ? 'opacity-100 max-w-xs duration-500 delay-100' : 'opacity-0 max-w-0 duration-0'}`;

  return (
    <>
    <nav className={`fixed bottom-0 md:left-0 md:top-0 left-0 right-0 h-auto md:h-full ${isBurgerOpen ? 'md:w-[15.2%] translate-y-0' : 'md:w-[5%] max-md:translate-y-[calc(100%-12px)]'} w-full bg-slate-900 md:border-r-2 border-slate-800 grid grid-cols-5 md:flex md:flex-col items-center md:items-start md:justify-start py-2.5 md:py-8 px-1 md:px-3 transition-all duration-300 z-50 border-t-2 border-slate-800 md:border-t-0 shadow-[0_-8px_30px_rgba(0,0,0,0.4)]`}>
      ...
      <div className="hidden md:flex flex-col w-full mb-10 gap-6 items-start pl-3">
        <button
          onClick={() => setIsBurgerOpen(!isBurgerOpen)}
          className="flex items-center text-slate-500 hover:text-white transition-colors"
        >
          <div className="p-2 rounded-lg bg-slate-800/50 hover:bg-slate-700 transition-all duration-300">
            <FaBars size={20} />
          </div>
        </button>
      </div>

      <div className="order-1 md:order-none flex justify-center md:justify-start md:w-full md:py-3">
        <Link to='/' className={linkStyle("/")} onClick={handleMobileClick}>
          <FaHouse className="text-lg md:text-[145%] min-w-[24px]" />
          <p className={textStyle}>Home</p>
          <p className="md:hidden text-[9px] uppercase font-bold tracking-wider">Home</p>
        </Link>
      </div>

      <div className="order-2 md:order-none flex justify-center md:justify-start md:w-full md:py-3">
      ...
      <div className="order-4 md:order-none flex justify-center md:justify-start md:w-full md:py-3">
        <Link to='/friends' className={linkStyle('/friends')} onClick={handleMobileClick}>
          <FaUserGroup className="text-lg md:text-[145%] min-w-[24px]" />
          <p className={textStyle}>Friends</p>
          <p className="md:hidden text-[9px] uppercase font-bold tracking-wider">Friends</p>
        </Link>
      </div>

      <hr className="hidden md:block w-full border-t-2 border-slate-700 my-2" />

      <div className="order-3 md:order-none flex justify-center md:justify-start md:w-full md:py-3">
        <Link to='/mainDictionary' className={linkStyle('/mainDictionary')} onClick={handleMobileClick}>
          <FaBook className="text-lg md:text-[145%] min-w-[24px]" />
      ...
      <div className="order-5 md:order-none flex justify-center md:justify-start md:w-full md:py-3">
